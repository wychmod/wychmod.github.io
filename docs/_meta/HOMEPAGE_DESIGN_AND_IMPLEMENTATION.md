...
- 9 大知识领域、项目实战模块、主线文档、归档、工具箱和项目维护入口。
...
生产页面若展示数字，必须在实现当日重新运行统计。若无法提供稳定自动来源，应使用“9 大知识领域”“持续更新”“项目实战已开放”等稳定描述，或直接隐藏数字。
...
| 进入项目实战 | 透明背景，浅色边框 | 跳转到 `#/md/10-项目实战/00-手写Spring/00-架构总览.md` |
...
- 使用横向文章行，不使用三张卡片。
- 序号由 `.home-recent-list` 上的 CSS counter 按 DOM 顺序自动生成（`decimal-leading-zero` 格式），HTML 中 `home-recent-num` 不写文本；维护时只增删 `.home-recent-row` 行，禁止手写编号（V1.11，修复手写序号重复）。
...
7. 增删条目直接增删 `.home-recent-row` 行即可，序号由 CSS counter 自动生成，不要手写编号。
...
| V1.20 | 2026-08-16 | 修复 | 页脚维护链接改两列网格（`.home-footer-links` `grid-template-columns: 1fr 1fr; column-gap: 32px`），摊平 10 条链接避免一列过长、视觉更宽；链接行内边距 17→12px、分隔线 `border-top` 只加前 2 条（顶部边界线保持完整）；移动端 ≤767 恢复单列；缓存号 `?v=20260816c` |
| V1.19 | 2026-08-16 | 修复 | 页脚进一步加宽并扩充内容：栏宽 `1.25fr/1fr/0.9fr → 1.3fr/1.05fr/0.95fr`、栏距 `64→72px`、上下内边距 `64/72→72/80px`、品牌栏 `padding-right 24→32px`；单个元素加宽——品牌简介/标签/座右铭 `max-width 400→460px`（简介字号 15→16px、行高 1.8）、标签高 28→34px 内边距加宽、统计数字 28→34px、维护链接行字号 15→16px 内边距 14→17px、联系按钮高 52→58px 内边距加宽；维护栏新增 4 个真实链接（终端使用指南 / AI 助手使用指南 / AI 故障排查 / AI 更新摘要）；联系栏新增明细行（Email wychmod@foxmail.com、微信 tots-and、QQ 545480453，`.home-footer-contact` 新样式，图标 mail/message-circle/messages-square）；缓存号 `?v=20260816b` |
| V1.18 | 2026-08-16 | 修复 | 页脚（卷一终）三栏布局参考知识索引优化：栏距 `48→64px`、栏宽改不对称 `1.25fr / 1fr / 0.9fr`（品牌栏加宽承载头像/简介/标签/座右铭/统计，联系栏收窄仅按钮与终端）、品牌栏加 `padding-right: 24px` 拉开与维护栏距离、品牌简介/标签/座右铭 `max-width 320→400px` 让内容填满栏宽、上下内边距 `56/64→64/72px`；新增 768–1100px 中屏断点：品牌栏独占整行、维护/联系两栏并排，避免平板三栏过挤（≤767 仍单列）；页脚边界标签文案 `END / 卷一终` → `END / 本卷终`；缓存号 `?v=20260816a` |
