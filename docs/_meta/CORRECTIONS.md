...
### 2026-08-30 #19 - 全站修改记录瘦身：正文只留最新记录，历史统一归档

**涉及文件**：
- 更新：`docs/md/` 下 64 篇主线文档（「修改记录」更名「最新修改记录」，仅保留最近一次修改日期的条目，并新增归档指引链接）
- 新增：`docs/_meta/CHANGELOG_HISTORY.md`（全量历史归档，255 条记录按分类、按文档分节）
- 同步：`AGENTS.md`（§5 Step 5、§6 文档结构模板、§7 维护规则、§8.1 验收清单、§10.5 措辞）与 `docs/_meta/HOMEPAGE_DESIGN_AND_IMPLEMENTATION.md`（2 处「文末修改记录」措辞）

**改正记录**：

| # | 类型 | 原文 | 改正 | 原因 |
|---|---|---|---|---|
| 97 | 风格 | 每篇主线文档文末维护全量「修改记录」表格，随修订不断变长（最多单篇 16 条） | 更名「最新修改记录」只保留最近一次修改日期的条目；255 条完整历史按分类迁入 `docs/_meta/CHANGELOG_HISTORY.md`，正文节末附归档链接 | 历史记录堆在正文末尾干扰阅读，且全史分散在 64 篇里难以总览；统一归档后正文保持轻量、历史有单一查询入口 |

**说明**：
- 迁移口径：以日期为单位，「最新」指最近一次修改日期的全部条目（同日多条全保留）；仅 22 篇单记录文档只做标题更名。
- 表格后有正文的 4 篇（00-MySQL数据库 / 20-消息队列 / 30-分布式协调与搜索 / 05-当前进度与未完待续）已核对后文完整保留。
- 迁移由一次性脚本完成并已删除；后续维护规则写入 AGENTS.md §5/§7，`docs/md/archive/` 未做任何修改。
