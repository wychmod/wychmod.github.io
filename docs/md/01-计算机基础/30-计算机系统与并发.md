...
### 6.7 SSE（Server-Sent Events）服务器推送事件

- **定义**：基于 HTTP 长连接的服务器到客户端**单向推送**技术；浏览器端 API 为 `EventSource`，服务器返回 `Content-Type: text/event-stream` 的流式响应。
- **报文格式**：纯文本，事件块之间用空行分隔；字段包括 `data`（消息内容，多行拼接）、`event`（自定义事件类型）、`id`（事件编号）、`retry`（建议重连间隔毫秒数）。
...
- **浏览器端**：`new EventSource(url)` 后监听 `onmessage` / `addEventListener('custom-event', ...)`；断线时浏览器**自动重连**，并通过 `Last-Event-ID` 请求头携带最后收到的 `id` 实现断点续传。
- **fetch 手动消费**：`EventSource` 只支持 GET 且不能自定义请求头，AI 流式输出场景（POST 请求）需用 `fetch` + `res.body.getReader()` 逐块读取，注意 chunk 可能截断事件，需按空行分块并维护缓冲区。
- **服务端要点**：设置 `Content-Type: text/event-stream` + `Cache-Control: no-cache`，写完事件记得 `\n\n`；需关闭框架/Nginx 的响应缓冲（`proxy_buffering off`），或定期发注释行 `: ping\n\n` 防止中间层掐断空闲连接。

| 对比维度 | 轮询 | SSE | WebSocket |
|---|---|---|---|
| 方向 | 客户端拉 | 服务器->客户端（单向） | 全双工（双向） |
| 协议 | HTTP | HTTP | 独立 WS 协议 |
| 数据格式 | 任意 | 文本（UTF-8） | 文本 + 二进制 |
| 自动重连 | 无 | 浏览器内置 | 需自己实现 |
| 断线续传 | 无 | 内置（Last-Event-ID） | 需自己实现 |
| 代理/防火墙友好度 | 高 | 高（普通 HTTP） | 较低 |

- **选型**：只需服务器推送（通知、行情、日志流、LLM 流式输出）选 SSE；需要双向低延迟（游戏、协同编辑）选 WebSocket；低频场景轮询也够。
- **局限**：单向、仅文本不能传二进制、HTTP/1.1 下浏览器同域 6 连接限制（HTTP/2 多路复用后已缓解）。

> 💡 补充：SSE 是当前 LLM 流式输出的事实标准——OpenAI、Anthropic 等的 API 在 `stream: true` 时均返回 `text/event-stream` 格式。因为 LLM 交互天然是"请求一次、持续吐 token"的单向流，SSE 完全够用且比 WebSocket 简单得多；ChatGPT/Claude 的打字机效果即基于此实现。

### 6.8 Web 安全

- **WASC 六大威胁**：认证、授权、客户端攻击、命令执行、信息泄露、逻辑攻击。
- **OWASP Top 10（2021 版）**：失效的访问控制、加密失败、注入、不安全设计、安全配置错误、易受攻击和过时的组件、身份识别和认证失败、软件和数据完整性故障、安全日志和监控失败、服务端请求伪造（SSRF）。
...
> **原文归档**：[archive/old-cs-notes/](/md/archive/README?id=old-cs-notes)
