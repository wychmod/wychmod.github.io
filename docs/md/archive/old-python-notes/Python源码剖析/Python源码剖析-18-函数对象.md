...
![](../../youdaonote-images/Pasted%20image%2020221214164538.png)

由此可见，调用 _fibonacci(10000)_ 时，栈帧链长度将达到 _10000_ 。因此，实现递归算法需要特别小心，不免栈内存溢出。

这是肯定的。_Python_ 内部通过 **内存池** 技术对频繁分配回收内存的场景进行了优化，栈帧频繁创建回收导致的性能问题得到一定的缓解。

> 为什么模块用字典存储常量(或属性)，而函数是用静态的名字空间呢？

> Python模块内的并不是常量，Python没有常量的说法。在模块内全局里定义的都是变量，全局变量是可修改的，所以放字典内。函数内的局部变量，每次加载函数时，这些局部变量的值都是一样的，这些值放在PyCodeObject里。加载执行函数的时候，先形成栈帧对象PyFrameObject，栈帧对象形成的时候会去PyCodeObject里读取函数的局部变量的值，每次对函数内局部变量的值进行修改，都是改动PyFrameObject从PyCodeObject加载的该变量的值，PyCodeObject里的这个值从来不变。