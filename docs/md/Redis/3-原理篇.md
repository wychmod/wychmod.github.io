...
> Redis会共享值为0到9999的字符串对象。初始化为创建，和pythonyi ya
...
![](../原理篇.assets/1653984583289.png)

那么Redis将在底层创建两个SDS，其中一个是包含“name”的SDS，另一个是包含“虎哥”的SDS。

Redis是C语言实现的，其中SDS是一个结构体，源码如下：

![](./原理篇.assets/1653984624671.png)

例如，一个包含字符串“name”的sds结构如下：

![](./原理篇.assets/1653984648404.png)

SDS之所以叫做动态字符串，是因为它具备动态扩容的能力，例如一个内容为“hi”的SDS：

![](./原理篇.assets/1653984787383.png)
...
![](./原理篇.assets/1653984822363.png)

![](./原理篇.assets/1653984838306.png)

![](../youdaonote-images/Pasted%20image%2020230207171220.png)
...
![](./原理篇.assets/1653984923322.png)

其中的encoding包含三种模式，表示存储的整数大小不同：

![](./原理篇.assets/1653984942385.png)

为了方便查找，Redis会将intset中所有的整数按照升序依次保存在contents数组中，结构如图：

![](./原理篇.assets/1653985149557.png)
...
![](./原理篇.assets/1653985276621.png)

源码如下：

![](./原理篇.assets/1653985304075.png)

![](./原理篇.assets/1653985327653.png)
...
![](./原理篇.assets/1653985396560.png)

当我们向Dict添加键值对时，Redis首先根据key计算出hash值（h），然后利用 h & sizemask来计算元素应该存储到数组中的哪个索引位置。我们存储k1=v1，假设k1的哈希值h =1，则1&3 =1，因此k1=v1要存储到数组角标1位置。

![](./原理篇.assets/1653985497735.png)

Dict由三部分组成，分别是：哈希表（DictHashTable）、哈希节点（DictEntry）、字典（Dict）

![](./原理篇.assets/1653985570612.png)

![](./原理篇.assets/1653985586543.png)

![](./原理篇.assets/1653985640422.png)
...
![](./原理篇.assets/1653985716275.png)

![](./原理篇.assets/1653985743412.png)
...
![](./原理篇.assets/1653985987327.png)

![](./原理篇.assets/1653986020491.png)
...
![](./原理篇.assets/1653986282879.png)

![](./原理篇.assets/1653986217182.png)
...
![](./原理篇.assets/1653986642777.png)

以下是QuickList的和QuickListNode的结构源码：

![](./原理篇.assets/1653986667228.png)

我们接下来用一段流程图来描述当前的这个结构

![](./原理篇.assets/1653986718554.png)
...
![](./原理篇.assets/1653986771309.png)

SkipList（跳表）首先是链表，但与传统链表相比有几点差异：
元素按照升序排列存储
节点可能包含多个指针，指针跨度不同。

![](./原理篇.assets/1653986813240.png)

SkipList（跳表）首先是链表，但与传统链表相比有几点差异：
元素按照升序排列存储
节点可能包含多个指针，指针跨度不同。

![](./原理篇.assets/1653986877620.png)
...
![](./原理篇.assets/1653987103450.png)

如果存储的字符串是整数值，并且大小在LONG_MAX范围内，则会采用INT编码：直接将数据保存在RedisObject的ptr指针位置（刚好8字节），不再需要SDS了。

![](./原理篇.assets/1653987159575.png)

![](./原理篇.assets/1653987172764.png)

![](./原理篇.assets/1653987202522.png)
...
![](./原理篇.assets/1653987388177.png)

结构如下：

​	![](./原理篇.assets/1653987454403.png)
...
![](./原理篇.assets/1653992121692.png)

![](./原理篇.assets/1653992172526.png)
...
![](./原理篇.assets/1653992238097.png)

![](./原理篇.assets/1653992299740.png)
...
![](./原理篇.assets/1653992339937.png)

zset集合如下：

![](./原理篇.assets/1653992360355.png)
...
![](./原理篇.assets/1653911877542.png)

#### 3.0.2 对比

最后用一幅图，来说明他们之间的区别

![](./原理篇.assets/1653912219712.png)
...
单行字符串：首字节是 ‘+’ ，后面跟上单行字符串，以CRLF（ "/r/n" ）结尾。例如返回"OK"： "+OK/r/n"

错误（Errors）：首字节是 ‘-’ ，与单行字符串格式一样，只是字符串是异常信息，例如："-Error message/r/n"

数值：首字节是 ‘:’ ，后面跟上数字格式的字符串，以CRLF结尾。例如：":10/r/n"

多行字符串：首字节是 ‘$’ ，表示二进制安全的字符串，最大支持512MB：

如果大小为0，则代表空字符串："$0/r/n/r/n"

如果大小为-1，则代表不存在："$-1/r/n"

数组：首字节是 ‘*’，后面跟上数组元素个数，再跟上元素，元素数据类型不限:

![](./原理篇.assets/1653982993020.png)
...
![](./原理篇.assets/1653983366243.png)

可以发现，当key的TTL到期以后，再次访问name返回的是nil，说明这个key已经不存在了，对应的内存也得到释放。从而起到内存回收的目的。

Redis本身是一个典型的key-value内存存储数据库，因此所有的key、value都保存在之前学习过的Dict结构中。不过在其database结构体中，有两个Dict：一个用来记录key-value；另一个用来记录key-TTL。

![](./原理篇.assets/1653983423128.png)



![](./原理篇.assets/1653983606531.png)
