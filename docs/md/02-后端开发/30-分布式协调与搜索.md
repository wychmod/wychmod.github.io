...
> 💡 补充：这些笔记形成于 2021-2023 年，对应的 Zookeeper 版本多为 3.5.7（当前最新为 3.9.x），Elasticsearch 版本集中在 7.12.1（当前最新为 9.x，8.x 仍在维护）。虽然核心原理变化不大，但生产环境选型时应以官方最新版本为准，并关注 Elastic 与 OpenSearch 的许可差异。
...
> 💡 补充：Zookeeper 3.5 之后源码结构已大幅调整，例如引入 `zookeeper-jute` 独立模块、提供可选的 Netty 服务端通信（默认仍为 NIO）等；阅读源码时建议结合具体版本的类图与调试断点。
...
> **原文归档**：[archive/old-distributed-coord-notes/](/md/archive/README?id=old-distributed-coord-notes)
