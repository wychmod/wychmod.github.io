...
> 💡 补充：这些笔记形成于 2021-2023 年，对应的 Zookeeper 版本多为 3.5.7，Elasticsearch 版本集中在 7.12.1。虽然核心原理变化不大，但生产环境选型时应以官方最新 LTS 版本为准，并关注 Elastic 与 OpenSearch 的许可差异。
...
ES 官方提供 Java Low Level 与 High Level Rest Client，原文以 **RestHighLevelClient** 为主：
...
- **脑裂（Split-Brain）**：因网络分区导致多个节点认为自己是 Master。ES 7.0 后通过默认配置 `discovery.zen.minimum_master_nodes` 要求选票超过 `(eligible节点数 + 1) / 2` 来避免。
...
5. **ES 脑裂与最小 Master 节点**：生产环境务必配置 `discovery.zen.minimum_master_nodes` 或依赖 ES 7+ 默认行为，并合理拆分节点角色。
