...
-- MySQL 8.0+ 使用 performance_schema（INNODB_LOCKS / INNODB_LOCK_WAITS 已移除）
...
- **组复制 MGR**：基于类 Paxos（Mencius）协议的分布式一致性复制，自动故障转移。
...
8. **自增锁模式**：`innodb_autoinc_lock_mode` 在 5.7 默认 1（连续），**8.0+ 默认 2**（交错，自增值可能不连续，但并发性能更好）。
