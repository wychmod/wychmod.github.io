...
> 💡 补充：原文未提及，下面是爬虫项目里最常见的几个坑：

1. **反爬触发**：请求频率过高被封 IP。解决：用代理池 + 随机 User-Agent + 限速。
2. **页面结构变化**：CSS 选择器突然失效。解决：用稳定的属性（`data-*`）而非位置。
3. **登录态过期**：cookie 失效。解决：用 Playwright 走真实浏览器流程。
4. **编码问题**：GBK / UTF-8 / GB2312 混杂。解决：统一用 `response.apparent_encoding` 探测。
5. **法律风险**：爬用户隐私/受版权保护内容。解决：别爬。
...
- **Playwright 官方文档**：https://playwright.dev/python/ — 现代浏览器自动化首选
- **httpx 文档**：https://www.python-httpx.org/ — 异步 HTTP 客户端，比 requests 更适合高并发
- **反爬策略白皮书**：各大平台的 robots.txt 和开放平台文档
- **数据合规指南**：《数据安全法》《个人信息保护法》原文
