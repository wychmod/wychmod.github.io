...
> 共 77 道高频题，答案均为“抓重点”式，详细推导请查看文末原内容存档。
...
| 28 | 什么是 Java 内存模型（JMM） | 定义多线程环境下内存访问规则的规范：变量存于主内存，线程各有私有工作内存（共享变量副本），不能直接读写主内存；解决 CPU 缓存一致性与指令重排序问题，保证原子性、可见性、有序性，靠 `volatile`/`synchronized`/`Lock` 实现同步。 |
| 29 | 什么是 CAS，ABA 问题如何解决 | CAS：比较并交换，乐观锁实现；ABA 问题用 `AtomicStampedReference` 或 `AtomicMarkableReference` 加版本号/标记解决。 |
| 30 | AQS 是什么 | `AbstractQueuedSynchronizer`，JUC 核心抽象类，维护 `state` 和 CLH 队列，是 `ReentrantLock`、`Semaphore`、`CountDownLatch` 等的基础。 |
| 31 | `ThreadLocal` 为什么会造成内存泄漏 | `ThreadLocalMap` 的 Entry 以 `ThreadLocal` 弱引用为 key，key 被 GC 后 value 仍被强引用，无法回收；建议 `static final` 或在 `finally` 中 `remove()`。 |
| 32 | 线程池 7 大参数 | `corePoolSize`、`maximumPoolSize`、`keepAliveTime`、`TimeUnit`、`workQueue`、`ThreadFactory`、`RejectedExecutionHandler`。 |
| 33 | 线程池运行流程 | 核心线程满 → 入队 → 队列满 → 扩容到最大线程 → 仍满则执行拒绝策略。 |
| 34 | 线程池拒绝策略 | AbortPolicy（抛异常）、CallerRunsPolicy（调用者执行）、DiscardPolicy（静默丢弃）、DiscardOldestPolicy（丢弃最老任务）。 |
| 35 | `ConcurrentHashMap` 如何保证线程安全 | JDK 1.7 分段锁 Segment；JDK 1.8 CAS + `synchronized` 锁头节点，粒度更小，扩容多线程协作。 |
| 36 | 死锁四个条件与解决 | 互斥、占有并等待、不可抢占、循环等待；解决：按统一顺序加锁、设置超时、死锁检测与恢复。 |
| 37 | 线程有哪些状态 | OS 层：运行态、就绪态、休眠态（阻塞是休眠的一种）；JVM 层 6 种：`NEW`、`RUNNABLE`（含运行+就绪）、`BLOCKED`、`WAITING`、`TIMED_WAITING`、`TERMINATED`。 |
| 38 | `CyclicBarrier` 与 `CountDownLatch` 区别 | 本质都是让一组线程同步到同一集合点；`CyclicBarrier` 可重复使用、不阻塞主线程、可获取线程数并在出错时处理；`CountDownLatch` 一次性、`await()` 能阻塞主线程。 |
| 39 | `wait` 与 `sleep` 区别 | `wait` 释放锁和 CPU，必须持有对象锁；`sleep` 只释放 CPU，不释放锁。 |
| 40 | 保证线程安全的手段 | 互斥锁（`synchronized`/`Lock`）、线程安全容器（`ConcurrentHashMap` 等）、无状态设计、`ThreadLocal`、原子类。 |
...
| # | 问题 | 关键答案 |
|---|------|---------|
| 41 | Spring Boot 自动装配原理 | `@EnableAutoConfiguration` → `AutoConfigurationImportSelector` → 读取 `META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`（Spring Boot 3.x；2.x 为 `spring.factories`）→ 条件装配 `@Conditional`。 |
| 42 | Spring 如何解决循环依赖 | 仅支持 setter/field 注入的循环依赖，通过三级缓存（singletonObjects/earlySingletonObjects/singletonFactories）暴露早期引用；构造器注入无法解决。 |
| 43 | `@Transactional` 事务失效场景 | 非 public 方法、内部调用、异常被 try-catch 吞掉、timeout 过小、数据库引擎不支持事务、未配置对非 RuntimeException 回滚。 |
| 44 | Spring MVC 工作流程 | `DispatcherServlet` → `HandlerMapping` → `HandlerAdapter` → 执行 Handler → `ModelAndView` → `ViewResolver` → 渲染视图返回客户端。 |
| 45 | Spring Bean 作用域 | `singleton`（默认）、`prototype`、`request`、`session`、`global-session`（Spring 5 已移除）。 |
| 46 | Spring Bean 的生命周期 | 创建 → 使用 → 销毁；`singleton` 随容器启动创建、容器销毁时销毁（延时加载则首次调用时创建）；`prototype` 调用前创建、调用后销毁，作用域决定生命周期长短。 |
| 47 | Spring Bean 线程安全吗 | 与 Spring 无关，取决于 Bean 是否有状态；无状态单例安全，有状态单例不安全。 |
| 48 | 拦截器与过滤器区别 | 过滤器属于 Servlet 规范，基于回调，先执行；拦截器属于 Spring，基于 AOP/反射，更接近业务。 |
| 49 | Spring 中常用的设计模式 | 工厂模式（`BeanFactory`）、单例模式（容器单例）、代理模式（AOP）、模板方法（`JdbcTemplate`）、观察者模式（事件监听）。 |

> 💡 补充：Spring Boot 简化了配置，但构建 Web 项目仍需 Spring MVC；Spring Cloud 解决分布式问题（注册发现、配置、熔断、网关等）。

---

## 六、MyBatis / MySQL / Redis / MQ

### MyBatis

| # | 问题 | 关键答案 |
|---|------|---------|
| 50 | MyBatis 执行流程 | 读取配置文件 → 创建 `SqlSessionFactory` → 创建 `SqlSession` → 按 Mapper 方法定位并执行 SQL（`#{}` 替换为实际参数值）→ 结果映射为 Java 对象返回。 |
| 51 | `#{}` 与 `${}` 的区别 | `#{}` 替换为参数值，无注入风险，优先使用；`${}` 直接拼入 SQL 语句，有安全风险，仅在参数充分验证后使用。 |
| 52 | MyBatis 二级缓存 | 一级缓存 `SqlSession` 级别，自带且无法关闭，跨 `SqlSession` 不生效；二级缓存 Mapper 级别，多个 `SqlSession` 共享，默认关闭需手动开启，可使用第三方实现（如 Ehcache）。 |

### MySQL

| # | 问题 | 关键答案 |
|---|------|---------|
| 53 | InnoDB 与 MyISAM 区别 | InnoDB 支持事务、行锁、MVCC、外键、崩溃恢复；MyISAM 表锁、不支持事务，适合读多写少。 |
| 54 | 事务 ACID 与隔离级别 | A 原子性、C 一致性、I 隔离性、D 持久性；隔离级别：RU、RC、RR（InnoDB 默认）、Serializable。 |
| 55 | MVCC 原理 | 多版本并发控制，依赖隐藏字段（DB_TRX_ID、DB_ROLL_PTR）、Undo Log、Read View 实现快照读，解决不可重复读和幻读。 |
| 56 | 索引为什么用 B+ 树 | B+ 树非叶子节点只存 key，叶子节点存数据并链表相连；单次 IO 装载更多索引，范围查询和顺序读取效率高。 |
| 57 | 索引失效常见原因 | 不满足最左前缀、对列做函数/运算、隐式类型转换、`is not null`、全表扫描更优、like `%xxx` 前缀模糊。 |
| 58 | 慢查询如何优化 | 开慢查询日志 → `EXPLAIN` 分析执行计划 → 加合适索引 → 优化 SQL → 分库分表/读写分离/缓存。 |

### Redis

| # | 问题 | 关键答案 |
|---|------|---------|
| 59 | Redis 为什么快 | 纯内存操作、单线程避免上下文切换、高效数据结构、IO 多路复用（epoll）、RESP 协议简单。 |
| 60 | Redis 有哪些数据类型 | 常用 5 种：`String`、`List`（简单消息队列）、`Hash`、`Set`（关注关系）、`ZSet`（带分数，存排名信息）；底层编码含 intset、dict（用于 rehash）、ZipList、QuickList、SkipList（跳表）。 |
| 61 | Redis 持久化方式 | RDB（快照，恢复快但可能丢数据）、AOF（日志，数据安全但文件大）、混合持久化（RDB + AOF）。 |
| 62 | 缓存穿透/击穿/雪崩 | 穿透：缓存与 DB 都不存在，用布隆过滤器/空值缓存；击穿：热点 key 过期，用互斥锁/逻辑过期；雪崩：大量 key 同时失效，加随机 TTL/集群/限流。 |
| 63 | 常见的缓存更新策略 | 旁路缓存（Cache Aside）：先更新 DB 再删除缓存，不一致概率小，但写频繁会频繁删 key，强一致场景可加锁更新缓存；读写穿透：读写都操作 cache，写时由 cache 写入数据库；异步缓存写入：不直接更新 db，采用异步方式更新。 |
| 64 | Redis 分布式锁 | `SET key value NX EX seconds`；防止误删用 Lua 判断 value；生产环境推荐 Redisson。 |
| 65 | Redis 集群模式 | 主从复制、哨兵模式（高可用）、Cluster 模式（分片 + 自动故障转移）。 |

### MQ

| # | 问题 | 关键答案 |
|---|------|---------|
| 66 | 消息丢失如何处理 | 生产者：重试/事务消息；Broker：同步刷盘 + 集群复制；消费者：业务处理完再 ack。 |
| 67 | 消息重复消费如何处理 | 业务幂等：数据库唯一索引、状态机去重、记录已消费消息 ID。 |
| 68 | 消息积压如何处理 | 定位消费慢原因；扩容队列和消费者；临时将消息转储到新队列并行消费；修复消费逻辑。 |
| 69 | 消息队列是推还是拉模式 | RocketMQ/Kafka 采用长轮询：消费者发起拉取请求，条件不满足时挂起，避免多次拉取动作，Broker 一有新消息立即返回，兼具推的及时性与拉的吞吐。 |
...
| # | 问题 | 关键答案 |
|---|------|---------|
| 70 | Spring 中用了哪些设计模式 | 工厂（BeanFactory）、单例（容器 Bean）、代理（AOP/JDK/CGLIB）、模板方法（JdbcTemplate）、观察者（ApplicationEvent）、适配器（HandlerAdapter）、装饰器（事务包装）。 |
| 71 | 单例模式实现方式 | 饿汉式、懒汉式（线程不安全）、DCL + `volatile`、`静态内部类`、`Enum`（最推荐，防反射和序列化破坏）。 |
| 72 | 代理模式分类 | 静态代理、JDK 动态代理（实现接口）、CGLIB 动态代理（继承类）；Spring AOP 默认策略：有接口用 JDK 动态代理，无接口用 CGLIB（Spring Boot 2.x+ 默认统一使用 CGLIB）。 |
...
| # | 问题 | 关键答案 |
|---|------|---------|
| 73 | 如何设计一个秒杀系统 | 页面静态化 + CDN；库存预热到 Redis；Redis 原子扣减库存；MQ 异步下单削峰；限流/答题/验证码防刷；数据库分库分表 + 乐观锁防超卖。 |
| 74 | 扫码登录流程 | Web 请求二维码 ID 并轮询；App 扫码后通知服务端变为“待确认”；用户确认后服务端生成正式 Token 返回 Web。 |
| 75 | 订单重复提交/支付 | 前端防抖；后端 token 幂等（一次 token 只能生成一个订单）；订单状态机 + 唯一索引保证支付幂等。 |
| 76 | 100 亿数据找 Top 1000 | 小顶堆 O(N log K)；或分治法：分片求各片 Top K，再汇总求全局 Top K；海量数据考虑 MapReduce。 |
| 77 | 服务器响应慢如何排查 | `top`/`uptime` 看 CPU 负载 → `ps` 定位线程 → 线程 ID 转十六进制 → `jstack` 抓取栈信息 → 分析热点代码/GC/死锁。 |
...
## 如何谈优点
韧性，研究生期间我最大的感受就是我韧性直线上升， 我要去自己学习跟未来工作有关的，跟科研有关的，还要操心实习，横向课题之类的东西。

有良好的自学能力，平常有读书阅读，了解新技术的习惯

有毅力不服输，要做好的东西加班熬夜多花时间也要做好
...
## 优惠券系统的完善
字节引入rocketmq 延时消息队列进行重复发送过期mq，redis分片，mysql 分库分表。多级缓存。

vivo 也是分库分表减轻压力，存库扣件没有使用redis，直接分库分表行锁扣减。定向发券，精准营销。统一领券。
...
![](../youdaonote-images/Pasted%20image%2020230920190022.png)

## 一张表，有一个自增的主键，我现在想不让他进行自增，应该如何做

1. 修改表结构：你可以通过修改表结构的方式，将自增主键的属性修改为非自增。
2. 删除自增属性
3. 设置自增步长为1

## 倒排索引
在倒排索引中，每个词都被视为一个关键词（Keyword），而每个关键词都维护一个包含该词的文档列表。文档列表中记录了包含该关键词的文档或记录的信息，如文档ID、位置等。
...
由于异步非阻塞框架基本为单线程运行，因此要利用协程实现事件循环。FastAPl推荐使用uvicorn来运行服务，uvicorn是基于uv1oop和httptools构建的闪电般快速的ASGl服务器。Python:3.5+的标准库asyncio提供了事件循环用来实现协程，并引入了async/,await关键字语法
以定义协程。FastAPI
则是利用了uvloop,相对于asyncio,更进一步地提升了速度。uvloop是用Cython编写的，并
建立在libuv之上。libuv是一种高性能的、跨平台异步的I/O类库，nodejs也使用到了它。
...
- 头部压缩(发出多个请求，他们的头是⼀样的或是相似的，那么，协议会帮你消除重复的部分。)
- ⼆进制格式
- 并发传输
- 服务器主动推送资源
...
QUIC 有以下 3 个特点。
- ⽆队头阻塞（当某个流发⽣丢包时，只会阻塞这个流，其他流不会受到影响，因此不存在队头阻塞问题。）
- 更快的连接建⽴(TLS包含在QUIC里，不用单独验证)
- 连接迁移（ QUIC 没有⽤四元组，⽽是通过连接 ID 来标记通信的两个端点，wifi切换的时候不会卡顿）
...
## tcp/ip 四层模型
![](../youdaonote-images/Pasted%20image%2020230924224929.png)
...
> 超时重传遵循指数退避的策略，1s 2s 4s 最后到32s一共加起来差不多一分钟吧
...
Redis 事务的本质是`一组命令的集合`。事务`支持一次执行多个命令`，一个事务中所有命令都会被序列化。在事务执行过程，会按照顺序串行化执行队列中的命令，其他客户端提交的命令请求不会插入到事务执行命令序列中。

![](../youdaonote-images/Pasted%20image%2020231101203658.png)

## Redis脑裂现象
- 当Redis主从集群环境出现两个主节点为客户端提供服务，这时客户端请求命令可能会发生数据丢失的情况。
![](../youdaonote-images/Pasted%20image%2020231004204707.png)

![](../youdaonote-images/Pasted%20image%2020231004205228.png)
...
1. 使用Redis的命令`SLOWLOG GET`可以获取Redis的慢查询日志。
	 可以通过`CONFIG SET slowlog-log-slower-than <microseconds>`来设置慢查询的阈值，默认为10000微秒（10毫秒）。
## Redis慢查询使用的是什么数据结构
Redis慢查询使用的是有序集合数据结构。具体来说，Redis会将每次执行的命令以及对应的执行时间作为有序集合的成员，执行时间作为分数。这样可以方便地根据执行时间进行排序和查询，从而找出慢查询命令。

## Redis的pipeline
Redis的Pipeline是一种批量执行命令的技术，可以在客户端发送多个命令给Redis服务器，而无需等待每个命令的响应。这样可以显著提高客户端与服务器之间的通信效率。
...
- 旁路缓存
	- 先更新db，再删除缓存。虽然也会出现缓存不一致的情况，但是对缓存的操作要快于db的操作，可能性很小。
...
hashmap可以是因为在单线程下，取出来的值可以用containskey来判断有没有这个key，但是多线程下，a线程因为没有nullkey返回null，而b线程存入了nullkey，如果交叉执行，a线程就会认为当前有nullkey。
...
> jwt的优点是调用服务器的cpu计算能力，减少对数据库的读取，jwt的缺点是无法废弃，当信息更新的时候只能进行重新签发，需要设置一个黑名单将旧的过期。过期时间需要自己判断。
## java的消息队列
...
- 元空间优点：
	- 永久代的大小是固定的，当加载的类信息、常量池等数据超过了永久代的大小时，就会导致内存溢出。而元空间的大小可以根据需要进行调整，不再受到固定大小的限制，同时元空间使用本地内存而不是jvm内存。
	- ### 1.提高稳定性，降低 OOM
...
## reentrantLock 的公平和非公平
默认是非公平的，默认会自旋，然后进入等待队列，aqs的实现。自旋就是抢锁。如果是公平的就不自旋抢锁了，直接进入等待队列，一个一个来获取。然后非公平的，效率高，在自旋的时候是内核态不用切换了。
...
**让线程通过自旋锁进入一个等待队列，这样可以节省大量自旋的时间。大多数情况下，自旋进入等待队列，比自旋锁获取锁临界区容易太多。**
...
## 守护线程
专门为用户线程提供服务的线程，他的生命周期依赖于用户线程。比如垃圾回收，jvm关闭时，垃圾回收也没有了意义。
...
1. 领取活动
	1. 查询是否有抽奖单避免重复发奖
	2. 查询活动账单，把活动基本信息存在redis里
	3. 对活动信息进行校验，看活动有没有过期，看库存是不是大于0
	4. 扣减活动库存，因为操作库存比较敏感，可以设计一个参与次数，先在redis增，如果超过了再恢复库存。然后进行滑块锁的扣减。
	5. 创建领取活动信息，因为活动可以参加多次，每条记录由用户ID和活动ID、活动领取次数共同决定，**每条记录由uuid唯一标识（防重,保证幂等性）**
...
3. 执行抽奖策略
	...
	5. 扣减库存 暂时采用数据库行级锁的方式进行扣减库存
4. 抽奖结果落库
	1. 编程式事务保证锁定抽奖领取记录(用户活动表变更状态)和保存抽奖信息的原子性(保存抽奖到的信息)
	2. 分库分表存入数据库
5. 发送 MQ 触发发奖流程
	1. 不同的奖品有不同的流程
6. 返回发奖结果
...
## 分布式如何体现
分布式指使用分布式技术栈【RPC、MQ、分库分表、分布式任务调度等】来实现系统的**水平扩展**，需要满足**提高系统吞吐量**时进行扩容。也就是**数据和程序可以不位于一个服务器**上，而是分散到多个服务器，以网络上分散分布的地理信息数据及受其影响的数据库操作为研究对象的一种理论计算模型。

## RedLock 红锁算法
使用多个 Redis 实例来实现分布式锁，这是为了保证单点故障时还可用

尝试从 N 个相互独立的 Redis 实例获取锁，如果一个实例不可用，应该尽快尝试下一个

计算获得锁所消耗的时间，只有当这个时间小于锁的过期时间，并且从大多数（ N / 2 + 1 节点推从选举) 实例中获取了锁，就认为锁获取成功

如果锁获取失败会到每个实例上释放锁

## 表的设计

活动、算法、规则、策略、用户、订单等各个领域

## 数据库 id 的小设计
有自增id，也有对外id，对外id是为了防止人家猜出来id了解你的情况，自增id是对内使用，也是一级索引。

## 冷启动和库存预热
冷启动：服务刚刚启动时， Redis 并没有缓存，如果所有商品数据都要在第一次查询时添加缓存会对 DB 造成不小的压力

库存预热：在实际开发中，可以利用大数据统计用户访问的热点数据，在项目启动时将热点数据提前查询并保存到 Redis 中
...
![](../youdaonote-images/Pasted%20image%2020231004211845.png)
...
# MQ
## 消息队列是推还是拉模式
rocketmq和kafka都是采用长轮询的机制，具体的做法是通过消费者等待消息，当有消息时Broker会直接返回消息，如果没有消息都采取延迟处理的策略，并且为了保证及时，都会在有新消息到来的时候提醒消息来了。

**拉取请求不满足条件的时候挂起，避免多次拉取动作，消息一到就提醒返回。**
## mq的消息积压如何处理
1. 定位消费慢的原因，看是不是bug
2. 新增队列数和消费者数，如果队列数太小，那么可以先把所有消息读取，重新装到另一个队列多的消息队列，这样可以有很多消费者进行消费。

## 消息丢失怎么办
一般从三个方面来进行描述：
1. 发送消息：可以有两种，一种是try catch 重试的方式，一种是消息事务的方式（先发送状态是half，如果可用再提交事务，哪个步骤出问题，broker回去回调访问接口，看是rollback还是提交。）
2. 消息存储：需要在刷盘之后再给生产者响应。同步刷盘+raft算法的集群
3. 消息消费：执行完真正的逻辑再返回ack和自动故障转，当集群中有个机器宕机了，其他的要继续处理。

## 消息重复怎么办
在各个环节都可能产生重复消息。
所以需要幂等这种方来解决。业务上：**数据库唯一性约束、记录关键key进行判断。**

## qmq优化后的消息事务
去哪儿团队开发的消息事务的模式，在业务的过程中会有一个消息mysql表进行写入，当你完成事务后会把这个消息也写入这个消息表，然后如果发送成功就会在表中删除这个消息，如果发送失败，就会有线程不断的扫描这个表，去帮助他发送成功。                                         

qmq比rocketmq好在可以在一个事物里发送多个消息，而rmq只能在一个事物里发一次消息。
...
## 扫码登陆
1. web向服务器请求二维码id，然后展示给前端。定时轮询。
2. app扫码以后，会给服务器发送消息，这时轮询到扫码，变成待确认。app获得临时token。
3. 确认以后，web端货的正式token，可以写入登陆认证程序。
...
## 如何查看日志文件中topK的错误日志？
我的做法是，写一个python脚本，去读取日志文件，然后正则匹配到日志中的错误，然后储存到list，然后对他进行cout统计。实际上直接存在dict，或者有序dict里也行。

## 一个超大的单向链表，如上亿个节点，如何实现时间复杂度较低的排序
...
1. 全排序
2. 小顶堆
3. 分治法 每个大分区前1000个(100亿个数分成100块，每个服务器处理一块，1亿个数分成100块，每个CPU处理一块。)
4. hash法去重
...
## 秒杀压力过大应该采取的策略
- 堆机器是不靠谱的
- 商品详情页高并发优化（数据静态化，多级缓存CDN NGINX Redis）
- 下单高并发优化
	- 秒杀答题
	- 基于Redis扣减库存
	- 基于Nginx过滤无效请求，可以通过redis判定，然后和zookeeper联动，有nginx对znode进行判定。
	- 基于RocketMQ高并发订单削峰
	- 独立部署秒杀系统
	- 避免数据库抗高并发请求，可以分库分表行锁抗

## 银行转账延迟 可能会是那些原因

1. 网络延迟：银行转账需要通过网络进行数据传输，如果网络延迟较高，数据在传输过程中会遇到延迟，导致转账操作的延迟。
    
2. 服务器负载：银行的转账系统可能会面临大量的并发请求，如果服务器负载过高，处理请求的速度会变慢，从而导致转账操作的延迟。
    
3. 数据库访问延迟：转账操作需要对数据库进行读写操作，如果数据库访问延迟较高，会导致转账操作的延迟。
    
4. 事务处理时间：银行转账通常会使用事务来保证数据的一致性和完整性，事务的处理时间可能会导致转账操作的延迟。
    
5. 第三方系统延迟：银行转账可能需要与其他系统进行交互，如支付网关、风控系统等，如果这些系统的响应时间较长，会导致整个转账流程的延迟。
    
6. 安全验证：为了保证转账操作的安全性，银行可能会进行多重验证，如身份验证、支付密码验证等，这些验证过程可能会引起转账操作的延迟。
    
7. 批量处理：银行转账可能会按批次进行处理，即将多个转账请求合并成一个批次进行处理，这种批量处理方式可能会导致转账操作的延迟。
...
## 虚拟内存的内存布局⼤概是什么样⼦?
![](../youdaonote-images/Pasted%20image%2020230928214454.png)


通过这张图你可以看到，⽤户空间内存，从低到⾼分别是 6 种不同的内存段：代码段，包括⼆进制可执⾏代码；
...
> 堆和⽂件映射段的内存是动态分配的。⽐如说，使⽤ C 标准库的malloc() 或者 mmap() ，就可以分别在堆和⽂件映射段动态分配内存。

## 操作系统保护模式和实模式，区别在哪

1. 实模式将整个物理内存看成分段的区域，程序代码和数据位于不同区域，系统程序和⽤户程序并没有区别对待，⽽且**每⼀个指针都是指向实际的物理地址**。⽤户程序的⼀个指针如果指向了系统程序区域或其他⽤户程序区域，并修改了内容，后果就很可能是灾难性的。再者，随着软件的发展，1M的寻址空间已经远远不能满⾜实际的需求了。
2. 在保护模式中，除了内存寻址空间⼤⼤提⾼；提供了硬件对多任务的⽀持；物理内存地址也不能直接被程序访问，程序内部的地址(虚拟地址)要由操作系统转化为物理地址去访问，程序对此⼀⽆所知。
## 零拷⻉

sendfile 系统调⽤实现了零拷⻉技术，零拷⻉技术的⽂件传输⽅式相⽐传统⽂件传输的⽅式，

减少了 2 次上下⽂切换和数据拷⻉次数，只需要 2 次上下⽂切换和数据拷⻉次数，就可以完

成⽂件的传输，⽽且 2 次的数据拷⻉过程，都不需要通过 CPU，2 次都是由 DMA 来搬运，

使⽤零拷⻉的项⽬有nginx、kafka。

![](../youdaonote-images/Pasted%20image%2020230928204836.png)
## 进程调度算法
...
## 内核级线程和用户级线程
![](../youdaonote-images/Pasted%20image%2020231002162610.png)

在这个模型中，Java自己维护一个线程表，同时通过操作系统API创造内核级线程。操作系统帮助Java把用户级线程和内核级线程对应起来。对JVM来说，主线程和各个子线程是平行的，公平竞争CPU资源，可以利用多核优势。

在java 1.1以前，一个进程只能对应一个内核线程，没有办法映射到多核上去，和现在的python一样。
