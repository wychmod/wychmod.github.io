...
### 3.6 Worktree：一份仓库，多个工作现场

Git 仓库的存储分两层：`.git` 目录存放所有版本数据（对象库、引用、配置），工作目录只是某个 commit 检出的文件快照。`git worktree` 允许**一个 `.git` 仓库同时挂多个工作目录**：

```bash
git worktree add ../myproject-hotfix hotfix-branch
```

执行后 Git 在 `.git/worktrees/<name>/` 下为每个 worktree 保存独立的 HEAD 和索引，新目录里只有一个 `.git` 文件（指向主仓库的指针），版本数据所有 worktree 共享同一份。

与复制项目目录（`cp -r`）或重新 `git clone` 一份相比：

| 对比维度 | 复制目录 / 重新 clone | worktree |
|---|---|---|
| 磁盘占用 | 全量复制历史，仓库越大越贵 | 只占工作区文件，历史零复制 |
| 历史同步 | 两边分叉，要靠 push/pull 对齐 | 同一对象库，一边 commit 另一边立刻可见 |
| 分支检出约束 | 两边可能检同一分支，改动互相干扰 | Git 强制同一分支只能被一个 worktree 检出 |
| 未跟踪文件 | 混在复制里容易搞乱 | 各 worktree 天然隔离 |
| hooks/配置 | 需手动同步 | 共享同一套 |

典型场景：开发到一半要紧急修 bug（不用 stash 半成品，开个 worktree 挂到 hotfix 分支去修）、长时间跑对比测试、大规模重构时主目录保留可运行的旧版本。局限：子模块和部分依赖 `.git` 是目录的工具链不兼容；删除 worktree 目录后要 `git worktree prune` 清理残留元数据。

**什么时候适合开 worktree 并行**：判据是——两件事能不能各自独立地被验收。能，就适合并行；一件事的验收依赖另一件的结果，就老老实实串行。
